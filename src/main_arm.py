...
cols = ['Inclination', 'Orientation', 'I', 'O', 'M1', 'M2', 'M3']
data = []
...
for inclination in range(5, 76, 10):
    # Orientation's repetition
    for orientation in range(5, 361, 1):
        ...
        for i in np.arange(0, 1):  # time sampling >> steps of 0.02
            ...
            data.append([inclination, orientation, incli, orient, motors.motorsArray[0].getPosition(
    ...
    # print(df)
    df.to_csv(
        '/home/sofia/SOFIA_Python/data/Data_2024/arm_trajectory_6.csv', index=False)
    # df.to_csv(
